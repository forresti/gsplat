...
template <uint32_t COLOR_DIM>
__global__ void rasterize_to_pixels_fwd_multiple_output_kernel(
    const uint32_t C, const uint32_t N, const uint32_t n_isects, const bool packed,
    const float2 *__restrict__ means2d,    // [C, N, 2] or [nnz, 2]
    const float3 *__restrict__ conics,     // [C, N, 3] or [nnz, 3]
    const float *__restrict__ colors,      // [C, N, COLOR_DIM] or [nnz, COLOR_DIM]
    ...
    // return if out of bounds
    // keep not rasterizing threads around for reading data
    bool done[OUTPUTS_PER_THREAD];
    char all_done = 0;

    for(uint32_t jplus=0; jplus<OUTPUTS_PER_THREAD; jplus++){
        bool inside = (i < image_height && (j+jplus) < image_width);
        done[jplus] = !inside;
        all_done += !inside;
    }
    ...
    // numerical precision so we use double for it. However double make bwd 1.5x slower
    // so we stick with float for now.
    float T[OUTPUTS_PER_THREAD];
    for(int jplus=0; jplus<OUTPUTS_PER_THREAD; jplus++){
        T[jplus] = 1.0f;
    }
    // index of most recent gaussian to write to this thread's pixel
    uint32_t cur_idx = 0;
    ...
    for (uint32_t b = 0; b < num_batches; ++b) {
        // resync all threads before beginning next batch
        // end early if entire tile is done
        if (__syncthreads_count(all_done) >= block_size) {
            break;
        }
        ...
        uint32_t batch_start = range_start + block_size * b;
        uint32_t idx = batch_start + tr;
        // TODO(fni): increase batch size; add an outer loop here.
        if (idx < range_end) {
            int32_t g = flatten_ids[idx]; // flatten index in [C * N] or [nnz]
            id_batch[tr] = g;
            const float2 xy = means2d[g];
        ...
        // process gaussians in the current batch for this pixel
        uint32_t batch_size = min(block_size, range_end - batch_start);
        // for (uint32_t t = 0; (t < batch_size) && !done[0]; ++t) {  // TODO(fni): figure out how to re-enable `done` correctly
        for (uint32_t t = 0; (t < batch_size); ++t) {  // TODO(fni): figure out how to re-enable `done` correctly
            if(all_done == OUTPUTS_PER_THREAD){
                break;
            }
            all_done = 0; // FIXME: handle the case where some are `!inside`
            ...
            for(uint32_t jplus=0; jplus<OUTPUTS_PER_THREAD; jplus++)
            {
                ...
                const float next_T = T[jplus] * (1.0f - alpha);
                if (next_T <= 1e-4) { // this pixel is done: exclusive
                    done[jplus] = true;
                    all_done += 1;
                    // TODO(fni): if all next_T in this thread are done, let's `break`
                    continue;
                }

                int32_t g = id_batch[t];
                const float vis = alpha * T[jplus];
                const float *c_ptr = colors + g * COLOR_DIM;
                PRAGMA_UNROLL
                for (uint32_t k = 0; k < COLOR_DIM; ++k) {
    ...
    for(uint32_t jplus=0; jplus<OUTPUTS_PER_THREAD; jplus++){
        ...
        int32_t pix_id_tmp = i * image_width + j_tmp;

        // TODO(fni) FIXME: why is this saving 1 px instead of 4 px?
        render_alphas[pix_id_tmp] = 1.0f - T[jplus];
        PRAGMA_UNROLL
        for (uint32_t k = 0; k < COLOR_DIM; ++k) {
        ...
        // index in bin of last gaussian in this pixel
        last_ids[pix_id_tmp] = static_cast<int32_t>(cur_idx);
    }
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> rasterize_to_pixels_fwd_multiple_output_tensor(
    ...
    switch (channels) {
    case 3:
        rasterize_to_pixels_fwd_multiple_output_kernel<3><<<blocks, threads, 0, stream>>>(
            C, N, n_isects, packed, (float2 *)means2d.data_ptr<float>(),
            (float3 *)conics.data_ptr<float>(), colors.data_ptr<float>(),
            opacities.data_ptr<float>(),
            backgrounds.has_value() ? backgrounds.value().data_ptr<float>() : nullptr,
...
template <uint32_t COLOR_DIM>
__global__ void rasterize_to_pixels_fwd_no_shmem_kernel(
    const uint32_t C, const uint32_t N, const uint32_t n_isects, const bool packed,
    const float2 *__restrict__ means2d,    // [C, N, 2] or [nnz, 2]
    const float3 *__restrict__ conics,     // [C, N, 3] or [nnz, 3]
    const float *__restrict__ colors,      // [C, N, COLOR_DIM] or [nnz, COLOR_DIM]
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> rasterize_to_pixels_fwd_no_shmem_tensor(
    ...
    switch (channels) {
    case 3:
        rasterize_to_pixels_fwd_no_shmem_kernel<3><<<blocks, threads, 0, stream>>>(
            C, N, n_isects, packed, (float2 *)means2d.data_ptr<float>(),
            (float3 *)conics.data_ptr<float>(), colors.data_ptr<float>(),
            opacities.data_ptr<float>(),
            backgrounds.has_value() ? backgrounds.value().data_ptr<float>() : nullptr,
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> rasterize_to_pixels_fwd_load_balance_v1_tensor(
    // Gaussian parameters
    const torch::Tensor &means2d,   // [C, N, 2] or [nnz, 2]
    const torch::Tensor &conics,    // [C, N, 3] or [nnz, 3]
    const torch::Tensor &colors,    // [C, N, channels] or [nnz, channels]
